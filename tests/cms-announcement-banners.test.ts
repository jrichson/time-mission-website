...
describe('CMS announcement banners', () => {
  it('registers a text-only Payload collection with deploy-gated wording', () => {
    ...
    const tickerBehaviorMigration = read('cms/migrations/20260605_101500_announcement_banner_ticker_behavior.ts');
    ...
    expect(tickerBehaviorMigration).toContain('enum_announcement_banners_ticker_behavior');
    expect(tickerBehaviorMigration).toContain('"ticker_behavior"');
  ...
  it('exposes safe ticker behavior to the public announcement view', () => {
    expect(announcementBannerViewForDoc(baseBanner)?.tickerBehavior).toBe('auto');
