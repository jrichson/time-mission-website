...
last_activity: 2026-05-06 -- Quick task 260506-ehf: generated 24 WebP companions (cwebp -q 78) for experience JPGs; wired <source srcset> into 20 <picture> blocks in missions.html; mid-run regex bug caught (filename mismatch) and fixed; npm run check exits 0
progress:
  total_phases: 11
  completed_phases: 11
  total_plans: 52
...
Last activity: 2026-05-06 -- Completed quick task 260506-mp7: implement RFC #12 — collapse 8 per-location page scripts behind shared `js/page-widgets.js` (`window.TMPage.initCity` + `initIndex` facade). Net −3,569 LoC; public surface frozen to 2 entry points; smoke 58/58 pass; CSP hashes auto-regenerated.
...
| 260506-j4s | Implement RFC #11 — deepen location store: hybrid of Design A storage fix + Design C FALLBACK kill. Add `TM.getSavedSlug()` + `TM.onChange()`; eliminate nav.js dual-writer (`js/locations.js` is now the sole writer of `tm_location`); migrate-once on legacy `timeMissionLocation` then `removeItem` permanently; inject `window.TM_DATA` inline via `src/components/SiteScripts.astro` making data synchronous (`TM.ready === Promise.resolve()`); delete FALLBACK constant + fetch+ready+synthetic-current machinery + 9 root *.html dual-writer leaks + `scripts/check-fallback-locations-sync.js` (-119 LoC); add `scripts/check-locations-architecture.js` as single-writer guard. Net ~290 LoC removed; public TM 14-method surface preserved; smoke 58/58 stays green; pre-existing hreflang-cluster verify failure deferred | 2026-05-06 | 3f0e0a2 | [260506-j4s-implement-rfc-11-deepen-location-store-a](./quick/260506-j4s-implement-rfc-11-deepen-location-store-a/) |
| 260506-ksu | UX fix for post-RFC-#11 UAT finding: promoted `.location-info-page` to PRIMARY button in `.location-info-actions` and relabeled "Full venue page" → "Visit <City>" (verb-first per UI design brain). Demoted `.location-info-book` ("Book Now") to secondary outline; `.location-info-contact` stays tertiary text link to enforce strict primary/secondary/tertiary hierarchy. Resolves discoverability gap where mobile users tapping a city in the location overlay couldn't find the navigate-to-venue CTA. Selectors unchanged. Smoke 58/58 (P0-7a green); CSP hashes regenerated identically (no inline script delta) | 2026-05-06 | fd02dd5 | [260506-ksu-ux-promote-full-venue-page-to-primary-bu](./quick/260506-ksu-ux-promote-full-venue-page-to-primary-bu/) |
| 260506-mp7 | Implement RFC #12 — collapse 8 per-location page scripts (~4,657 LoC, near-identical) behind shared `js/page-widgets.js` exposing `window.TMPage.initCity` + `window.TMPage.initIndex` (12 widget init fns: typewriter, minutes, points, carousel, tilt, FAQ, testimonials, ticker, reveal, smooth-scroll, game-popup-disabled-but-wired). Migrated 8 Astro pages (7 cities + index) to `window.__TM_PAGE_INIT__` config-stash + deferred widgets script (mirrors existing `__TM_DATA__` hand-off). Auto-fixed defer/inline ordering bug caught by smoke (synchronous inline runs before deferred external — inverted to stash pattern). CSP hashes auto-regenerated by `inject-csp-hashes.mjs`. Net **−3,569 LoC**; public surface frozen to 2 entry points; `window.updateEyebrowLocation` registered internally; smoke 58/58 pass; hreflang-cluster pre-existing failure unchanged | 2026-05-06 | 7438289 | [260506-mp7-implement-rfc-12-collapse-per-location-p](./quick/260506-mp7-implement-rfc-12-collapse-per-location-p/) |
