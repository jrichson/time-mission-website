...
Last activity: 2026-05-06 -- Completed quick task 260506-j4s: implemented RFC #11 location store deepening — added TM.getSavedSlug + TM.onChange, fixed nav.js dual-writer (single writer = js/locations.js), migrate-once on legacy timeMissionLocation key, inline TM_DATA injection via SiteScripts.astro making data synchronous (TM.ready === Promise.resolve()), deleted FALLBACK constant + check-fallback-locations-sync.js (~290 LoC removed), public TM API surface preserved; smoke 58/58 still passing
