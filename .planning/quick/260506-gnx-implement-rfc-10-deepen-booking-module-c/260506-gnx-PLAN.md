...
<task type="auto">
  ...
  <action>
**Step 0 — GitNexus impact (MANDATORY per project guidance):**
Run, in order:
...
```
or with leading slash. Use a single sed pass scoped to root + groups/ (skip dist/, node_modules/, local tooling/, .planning/):
```bash
# List target files
find . -maxdepth 2 -name "*.html" \
  -not -path "./dist/*" -not -path "./node_modules/*" \
  -not -path "./local tooling/*" -not -path "./.planning/*" \
  -not -path "./_archive/*" \
...
grep -rln "roller-checkout" --include="*.html" --include="*.astro" --include="*.txt" . \
  | grep -v node_modules | grep -v dist/ | grep -v local tooling/ | grep -v .planning/
...
**Step 9 — Run the verification gate (per project guidance):**
```bash
npm run check
npm run test:smoke
```
...
**Constraints / things to AVOID:**
- Do NOT touch dist/ — it is build output and is regenerated by the Astro build. project guidance GitNexus rules already exclude dist via .gitignore patterns; if dist/ files appear in detect_changes, it means dist is checked in — pause and ask the user.
...
- Do NOT delete `window.BookingController` from booking-controller.js (already preserved in Task 1).
  ...
  <done>
js/roller-checkout.js deleted; zero references remain in src HTML/Astro/partials/scripts (verified via `grep -rln "roller-checkout" --include="*.html" --include="*.astro" --include="*.txt" --include="*.cjs" --include="*.mjs" --include="*.js"` excluding dist/, node_modules/, local tooling/, .planning/).
...
<verification>
End-to-end gate (per project guidance Verification gate):
