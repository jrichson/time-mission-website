/** Minifies copied CSS and JS assets in dist/ after Astro builds. */
...
async function minifyFile(file) {
  ...
  if (result.code.length >= original.length) {
    return { file, before: original.length, after: result.code.length, skipped: true };
  }
