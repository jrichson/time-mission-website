...
// sitemap.xml is emitted by src/pages/sitemap.xml.ts at build time.
// llms.txt and llms-full.txt are emitted by src/pages/*.txt.ts.
...
// Remove stale copy so src/pages/sitemap.xml.ts is not shadowed by prior root sitemap syncs.
...
if (tmMediaBaseFromEnv()) {
  const videoPlan = planVideoArtifacts({ mediaBase: tmMediaBaseFromEnv() });
