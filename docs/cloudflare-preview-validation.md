...
| D1 form archive | Before form testing, create the D1 database, apply `migrations/0001_form_submissions.sql`, and bind it as `FORM_SUBMISSIONS_DB` | Contact and newsletter submissions insert into `form_submissions`; production forms do not silently succeed without the archive binding | `tests/form-handler.test.mjs` |
| Contact form | Submit `/contact` with a valid Turnstile challenge; repeat once with a location that has a `CONTACT_TO_EMAIL_<LOCATION>` secret configured, such as `CONTACT_TO_EMAIL_WEST_NYACK` | Redirects to `/contact-thank-you`; D1 receives a `contact` row; the location-specific inbox receives the Resend email when configured, otherwise `CONTACT_TO_EMAIL` receives it | `npm run test:unit` |
| Newsletter form | Submit a representative newsletter form | Redirects to `/contact-thank-you`; D1 receives a `newsletter` row; configured `NEWSLETTER_TO_EMAIL` inbox receives a Resend email | `npm run test:unit` |
