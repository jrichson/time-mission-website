...
| Contact form | Submit `/contact` with a valid Turnstile challenge | Redirects to `/contact-thank-you`; D1 receives a `contact` row; configured `CONTACT_TO_EMAIL` inbox receives a Resend email | `npm run test:unit` |
