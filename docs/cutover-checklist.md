...
| FORM-01 | Web Dev | Cloudflare Pages Functions form backend | Configure Pages Function secrets: `FORM_EMAIL_API_KEY`, `FORM_FROM_EMAIL`, `CONTACT_TO_EMAIL`, `NEWSLETTER_TO_EMAIL`, `TURNSTILE_SECRET_KEY`; set public build var `PUBLIC_TURNSTILE_SITE_KEY`; create a D1 database, apply `migrations/0001_form_submissions.sql`, and bind it as `FORM_SUBMISSIONS_DB`; bind a Cloudflare KV namespace as `FORM_RATE_LIMIT_KV` for cross-isolate rate limiting; deploy with Wrangler from repo root so `functions/` uploads with `dist/`; submit contact + newsletter forms on preview and confirm D1 rows, inbox delivery, plus repeat-request `429`. | [ ] not started |
