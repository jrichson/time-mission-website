...
Checkout and deploy from annotated tag **`pre-astro-migration-baseline`**. See `.planning/baseline/README.md` for the pinned commit SHA.
...
- **Redirect loops** — Historical or canonical redirects recurse or prevent reaching real pages (`curl -sI -L --max-redirs 10` never spins).
...
3. Deploy the **static site root** from the rollback ref using your existing pipeline.
...
2. **Rollback deploy:** Attach production or preview deploy to the artifact built from the rollback git ref.
