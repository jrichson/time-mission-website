...
import {
  BRUSSELS_BACK_TO_SCHOOL_ANNOUNCEMENT_SNAPSHOT,
  BRUSSELS_BACK_TO_SCHOOL_PAGE_SNAPSHOT,
} from '../migration-data/20260902_brussels_back_to_school_sale_snapshot';

export async function up({ db }: MigrateUpArgs): Promise<void> {
  for (const page of BRUSSELS_BACK_TO_SCHOOL_PAGE_SNAPSHOT) {
  ...
  for (const announcement of BRUSSELS_BACK_TO_SCHOOL_ANNOUNCEMENT_SNAPSHOT) {
    await db.execute(sql`
      WITH inserted AS (
